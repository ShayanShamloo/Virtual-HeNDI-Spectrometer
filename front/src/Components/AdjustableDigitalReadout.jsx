...
function AdjustableDigitalReadout ({val, digits, id, className, name, unit, onChange, increment, decrement}) {
    
    let adjustedVal = String(Number(val).toFixed(2)).padStart(digits || 6,'0')
    return (
        <div id={id} className={className + ' digital-readout-adjustable'}>
            <input className='digital-readout' name={name} value={adjustedVal} unit={unit} digits={digits || 6} onChange={onChange} style={{width: `${((digits || 6)+1)/2}em`}}/>
